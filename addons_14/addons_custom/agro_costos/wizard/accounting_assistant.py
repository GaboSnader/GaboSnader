...
class WizardAccountingAssistant(models.TransientModel):
    ...
    date_inicio = fields.Date(string="Fecha inicio")
    date_fin = fields.Date(string="Fecha fin")

    branch_ids = fields.Many2many('stock.warehouse', string="Sucursal")
    negocio = fields.Many2many('costos.negocio', string="Negocio")
    area = fields.Many2many('costos.area', string="Area")
    department = fields.Many2many('costos.departamento', string="Departamento")
    equipo = fields.Many2many('costos.equipo', string="Equipo")
    tipo_operacion = fields.Many2many('costos.tipo_operacion', string="Tipo de Operación")
    partner_id = fields.Many2one('res.partner', string="Empresa")
    product_id = fields.Many2one('product.product', string="Producto")
    agrupamiento = fields.Selection(
        [('ninguna', 'Ninguna'), ('cuenta', 'Por cuenta'), ('empresa', 'Por empresa'), ], 'Agrupar por', default='ninguna')
    ...
    def send(self):
        ...
        for mv in mv_ids:
            if mv.parent_state == 'posted': #este parámetro no se pudo poner en la búsqueda, tal vez es calculado
        ...
        #move_ids.sort() #Ordenar elementos para pasarlos a la vista
        #print('Primero:',move_ids[0])
        #print('Ultimo:',move_ids[-1])
        move_objetcs_ids.sort(key=operator.attrgetter('id'))
        move_objetcs_ids.sort(key=operator.attrgetter('date')) #Esto porque puede haber elementos con un id posterior pero con fecha anterior a un registro previo
        for m in move_objetcs_ids: #La lista que se pasa como dominio a la vista se llena en esta parte, para poder tenerla ordenada por fecha
            move_ids.append(m.id)
        #print('PrimeroO:',move_objetcs_ids[0].id)
        #print('UltimoO:',move_objetcs_ids[-1].id)
        if self.agrupamiento == 'ninguna':
            ...
            print('Agrupando por cuenta')
            move_objetcs_ids.sort(key=operator.attrgetter('account_id.id')) # YA estaban ordenados x id y fecha, ahora se ordenan por cuenta
            ...
            print('Agrupando por Empresa')
            move_objetcs_ids.sort(key=operator.attrgetter('partner_id.id')) # YA estaban ordenados x id y fecha, ahora se ordenan por empresa
    ...
    def get_saldos(self,movimento_ids, grupo):
        if movimento_ids:
            ...
            if len(movimento_ids) == 1:
                primer_saldo = self.get_primer_saldo_inicial(movimento_ids[0].date, False, False)
                ...
                primer_saldo = self.get_primer_saldo_inicial(movimento_ids[0].date, movimento_ids[0].account_id.id, False)
                print('PRIMER SALDO Inicial x CUENTA::',primer_saldo)
                movimento_ids[0].saldo_inicial = primer_saldo #obtiene el primer saldo
                self.get_saldo_inicial_final(movimento_ids[0])
                print('PRIMER SALDO INICIAL x Cuenta:: id - %i , es de:: %d' % (movimento_ids[0].id, movimento_ids[0].saldo_inicial))
                for n in range(len(movimento_ids)):
                    if n>0: #Si el elemento actual tiene la misma cuenta que el anterior, se pone el Saldo inicial como el Saldo final del registro anterior
                        if movimento_ids[n].account_id.id == movimento_ids[n-1].account_id.id:
                            ...
                            print('id - %i ' % (movimento_ids[n].id))
                            primer_saldo_grupo = self.get_primer_saldo_inicial(movimento_ids[n].date, movimento_ids[n].account_id.id, False)
                            movimento_ids[n].saldo_inicial = primer_saldo_grupo
                            self.get_saldo_inicial_final(movimento_ids[n])
            elif len(movimento_ids) > 1 and grupo=='empresa':
                primer_saldo = self.get_primer_saldo_inicial(movimento_ids[0].date, False, False)
                print('PRIMER SALDO Inicial x EMPRESA::',primer_saldo)
                movimento_ids[0].saldo_inicial = primer_saldo #obtiene el primer saldo
                self.get_saldo_inicial_final(movimento_ids[0])
                print('PRIMER SALDO INICIAL x Empresa:: id - %i , es de:: %d' % (movimento_ids[0].id, movimento_ids[0].saldo_inicial))
                for n in range(len(movimento_ids)):
