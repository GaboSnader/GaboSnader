...
class CustomSurveyUserScore(Survey):
    ...
    def get_level(self,results):
        for r in results:
            for c in r['category']:
                c['level'] = self.determine_score(c['nullo'],c['low'],c['medium'],c['high'],c['very_high'],c['total'])
            for d in r['domain']:
                d['level'] = self.determine_score(d['nullo'],d['low'],d['medium'],d['high'],d['very_high'],d['total'])
            for di in r['dimension']:
                di['level'] = self.determine_score(di['nullo'],di['low'],di['medium'],di['high'],di['very_high'],di['total'])
        return r

    def determine_score(self,nullo,low,medium,hight,very_high,score):
        class_score = ''
        if score < nullo:
            class_score = "bg-dark" # "Nulo o despreciable"
        elif nullo <= score < low:
