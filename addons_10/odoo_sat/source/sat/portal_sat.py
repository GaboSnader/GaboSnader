...
class PortalSAT(object):
    ...
    def _init_values(self, target):
        ...
        #~ self.is_connect = self._login(ciec)
        return
    ...
    def _response(self, url, method='get', headers={}, data={}):
        ...
        try:
            if method == 'get':
                ...
                result = self._session.post(url, data=data, headers=headers, timeout=TIMEOUT)
            return result.text
    ...
    def login(self, ciec, from_script):
        #~ ERROR = 'El RFC o contraseña son incorrectos'
        ERROR = '<!--CAPTCHA-->'
        args = '?id=SATUPCFDiCon&sid=0&option=credential&sid=0'
        msg = 'Identificandose en el SAT'
        log.info(msg)
        response = self._response(self.URL_LOGIN + args, 'post')
        if self.not_network:
            return False
        ...
        data = {
            ...
            'Ecom_Password': ciec,
            'submit': 'Enviar',
            'jcaptcha': self._get_captcha(from_script),
        }
    ...
    def _get_captcha(self, from_script):
        ...
        result = self._session.get(url, timeout=TIMEOUT)
        return captcha.resolve(result.content, from_script)
